...
public class Lidar{
	...
	public double getSimulationDistance(int deltaTime) {
		...
		double distanceInCM;
		if(ran.nextFloat() <= 0.02f) { // 2% of the time, not getting an answer
			distanceInCM = 0;
		} else {
			distanceInCM = getDistance(deltaTime);
			distanceInCM += (int)ran.nextInt(WorldParams.lidarNoise*2) - WorldParams.lidarNoise; // +- 2 CM to the final calc
		}
