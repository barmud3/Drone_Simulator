	...
	*/
	...
	public void ai(int deltaTime) {
		...
		if(is_init) {
			...
			points.add(dronePoint); // adding the point of direction
...
//			doLeftRight();
		...
		if(SimulationWindow.return_home) {
			...
			if( Tools.getDistanceBetweenPoints(getLastPoint(), dronePoint) <  max_distance_between_points) {
				if(points.size() <= 1 && Tools.getDistanceBetweenPoints(getLastPoint(), dronePoint) <  max_distance_between_points/2.25) {
					speedDown();
				} else {
					removeLastPoint();
				}
		...
		if(!is_risky) {
			...
			if(!try_to_escape) {
				...
				if(a > 270 && b > 270) {
					...
					if(SimulationWindow.return_home) {
						if( Tools.getDistanceBetweenPoints(getLastPoint(), dronePoint) <  max_distance_between_points/2.25) {
							removeLastPoint();
						}
					} else {
						if( Tools.getDistanceBetweenPoints(getLastPoint(), dronePoint) >=  max_distance_between_points) {
